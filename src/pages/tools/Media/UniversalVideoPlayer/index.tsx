...
const UniversalVideoPlayer: React.FC = () => {
        ...
        flv: function (video: HTMLMediaElement, url: string) {
          if (mpegts.isSupported()) {
            const flvPlayer = mpegts.createPlayer({
              type: url.includes(".flv") ? "flv" : "mpegts",
              url: url,
              isLive: true,
            });
            ...
            message.error("FLV is not supported in this browser");
          }
        },
      },
      settings: [
  ...
  const isMixedContent = (videoUrl: string): boolean => {
  ...
  const handlePlayUrl = () => {
    ...
    if (isMixedContent(url)) {
      message.error({
        content: intl.formatMessage({ id: "tools.universalVideoPlayer.mixedContentError" }),
        duration: 8,
      });
      return;
    }
        ...
        <div className="bg-slate-50 dark:bg-slate-900/40 px-4 py-1 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <Text type="secondary" className="text-[10px] uppercase font-mono tracking-wider opacity-50">
              {url.includes('blob:') ? 'Local File' : url.includes('.m3u8') ? 'HLS' : url.includes('.mpd') ? 'DASH' : url.includes('.flv') ? 'FLV' : 'Direct'}
            </Text>
          </div>
        </div>
        <div
