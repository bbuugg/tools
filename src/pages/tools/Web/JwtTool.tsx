...
import { Card, Input, Button, Select, Tabs, Typography, Space, Row, Col, Alert, message, Tag } from 'antd';
import {
    CopyOutlined,
    DownloadOutlined,
    KeyOutlined,
...
const importKey = async (secret: string, alg: string) => {
    return window.crypto.subtle.importKey(
        ...
        { name: 'HMAC', hash: { name: alg === 'HS256' ? 'SHA-256' : alg === 'HS384' ? 'SHA-384' : 'SHA-512' } },
        false,
        ['sign', 'verify']
    );
};

const sign = async (header: string, payload: string, secret: string, alg: string = 'HS256'): Promise<string> => {
...
const verify = async (token: string, secret: string): Promise<boolean> => {
    ...
    return window.crypto.subtle.verify('HMAC', key, signature as any, data);
};
...
const JwtTool: React.FC = () => {
    ...
    // Tabs: encode, decode, verify
    const [activeTab, setActiveTab] = useState('encode');

    // Encode State
    const [encodeHeader, setEncodeHeader] = useState(JSON.stringify({ alg: "HS256", typ: "JWT" }, null, 2));
    const [encodePayload, setEncodePayload] = useState(JSON.stringify({ sub: "1234567890", name: "John Doe", iat: Math.floor(Date.now() / 1000) }, null, 2));
    ...
    const handleGenerate = async () => {
        try {
            ...
            const signature = await sign(headerB64, payloadB64, encodeSecret, encodeAlg);

            setGeneratedToken(`${headerB64}.${payloadB64}.${signature}`);
            message.success(intl.formatMessage({ id: 'tools.jwt.tokenGenerated' }));
        } catch (err: any) {
            message.error(intl.formatMessage({ id: 'tools.jwt.generationFailed' }) + ': ' + err.message);
        }
    };
    ...
    const handleDecode = async () => {
        ...
        try {
            ...
            if (payload.exp && payload.exp < Date.now() / 1000) {
                message.warning(intl.formatMessage({ id: 'tools.jwt.tokenExpired' }), 2);
            }
    ...
    const handleVerifySubmit = async () => {
        if (!verifyToken || !verifySecret) {
            message.error(intl.formatMessage({ id: 'tools.jwt.tokenSecretRequired' }));
            return;
        }
        try {
            const isValid = await verify(verifyToken.trim(), verifySecret);
            setVerifyResult(isValid ? 'valid' : 'invalid');
            if (isValid) message.success(intl.formatMessage({ id: 'tools.jwt.verified' }));
            else message.error(intl.formatMessage({ id: 'tools.jwt.invalidSignature' }));
        } catch (err) {
    ...
    const items = [
        {
            key: 'encode',
            label: intl.formatMessage({ id: 'tools.jwt.tab.encode' }),
            children: (
                <div className="space-y-2">
                    <Row gutter={24}>
                        <Col span={12}>
                            <Card title={<FormattedMessage id="tools.jwt.header" />} size="small" className="bg-white/5 border-slate-700">
                                <TextArea
                        ...
                        <Col span={12}>
                            <Card title={<FormattedMessage id="tools.jwt.payload" />} size="small" className="bg-white/5 border-slate-700">
                                <TextArea
                                    className="font-mono text-xs border-slate-700"
                                    rows={6}
                                    value={encodePayload}
                    ...
                    <Card title={<FormattedMessage id="tools.jwt.signing" />} size="small" className="bg-white/5 border-slate-700">
                        <Space orientation="vertical" className="w-full">
                            <Row gutter={16}>
                                <Col span={16}>
                                    <Text className="text-slate-400 block mb-1"><FormattedMessage id="tools.jwt.secret" /></Text>
                                    <Input.Password
                                        value={encodeSecret}
                                ...
                                <Col span={8}>
                                    <Text className="text-slate-400 block mb-1"><FormattedMessage id="tools.jwt.algorithm" /></Text>
                                    <Select
                                        value={encodeAlg}
                            ...
                            <Alert
                                title={
                                    <div className="flex justify-between items-start break-all">
                                        <Text className="font-mono text-green-400 text-sm">{generatedToken}</Text>
                                        <Space className="ml-4 shrink-0">
                                            <Button type="text" icon={<CopyOutlined />} onClick={() => copy(generatedToken)} />
                                            <Button type="text" icon={<DownloadOutlined />} onClick={() => {
                                                const blob = new Blob([generatedToken], { type: 'text/plain' });
                                                const url = URL.createObjectURL(blob);
                                                const a = document.createElement('a'); a.href = url; a.download = 'token.jwt'; a.click();
                                            }} />
                                        </Space>
                                    </div>
                                }
        ...
        {
            key: 'decode',
            label: intl.formatMessage({ id: 'tools.jwt.tab.decode' }),
            children: (
                <div className="space-y-6">
                    <Card className="bg-white/5 border-slate-700" size="small">
                        <Text className="text-slate-400 block mb-2"><FormattedMessage id="tools.jwt.tokenLabel" /></Text>
                        <TextArea
                            value={decodeToken}
                            onChange={(e) => setDecodeToken(e.target.value)}
                            rows={4}
                            className="font-mono text-xs border-slate-700 mb-4"
                            placeholder={intl.formatMessage({ id: 'tools.jwt.tokenPlaceholder' })}
                        />
                        <Space className="w-full justify-between">
                            <Input
                                placeholder={intl.formatMessage({ id: 'tools.jwt.secretPlaceholder' })}
                                value={decodeSecret}
                                onChange={e => setDecodeSecret(e.target.value)}
                                className="max-w-md border-slate-700"
                                prefix={<KeyOutlined className="text-slate-500" />}
                            />
                            <Button type="primary" onClick={handleDecode}><FormattedMessage id="tools.jwt.decodeVerify" /></Button>
                        </Space>
                    </Card>
                        ...
                        <Row gutter={24}>
                            <Col span={12}>
                                <div className="mb-2 flex items-center justify-between">
                                    <Text className="font-medium"><FormattedMessage id="tools.jwt.header" /></Text>
                                    {decodeSecret && decodeStatus !== 'unknown' && (
                                        <Tag color={decodeStatus === 'valid' ? 'success' : 'error'}>
                                            {decodeStatus === 'valid' ? intl.formatMessage({ id: 'tools.jwt.verified' }) : intl.formatMessage({ id: 'tools.jwt.invalidSignature' })}
                                        </Tag>
                                    )}
                                </div>
                                <TextArea
                                    ...
                                    value={decodedHeader}
                                    rows={10}
                                    className="font-mono text-xs border-slate-700 text-blue-300"
                                />
                            </Col>
                            <Col span={12}>
                                <div className="mb-2">
                                    <Text className="font-medium"><FormattedMessage id="tools.jwt.payload" /></Text>
                                </div>
                                <TextArea
                                    readOnly
        ...
        {
            key: 'verify',
            label: intl.formatMessage({ id: 'tools.jwt.tab.verify' }),
            children: (
                <div className="space-y-6">
                    <Card className="bg-white/5 border-slate-700">
                        ...
                        <div className="mb-6">
                            <Text className="text-slate-400 block mb-2"><FormattedMessage id="tools.jwt.secret" /></Text>
                            <Input
                                value={verifySecret}
                        ...
                        <Button type="primary" block onClick={handleVerifySubmit} size="large"><FormattedMessage id="tools.jwt.verifyButton" /></Button>
                    </Card>
                    ...
                    {verifyResult && (
                        <Alert
                            ...
                            message={verifyResult === 'valid' ? intl.formatMessage({ id: 'tools.jwt.verified' }) : intl.formatMessage({ id: 'tools.jwt.invalidSignature' })}
                            description={verifyResult === 'valid' ? intl.formatMessage({ id: 'tools.jwt.validDesc' }) : intl.formatMessage({ id: 'tools.jwt.invalidDesc' })}
                            className={verifyResult === 'valid' ? 'bg-green-900/10 border-green-500/30 text-green-300' : 'bg-red-900/10 border-red-500/30 text-red-300'}
                        />
                    )}
                </div>
            )
