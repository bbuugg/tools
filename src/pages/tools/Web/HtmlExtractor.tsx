...
import { Button, Card, Checkbox, Col, Divider, Empty, Image, Input, message, Row, Space, Tag, Typography } from 'antd';
                ...
                doc.querySelectorAll('video').forEach((video) => {
                    ...
                    if (src) {
                        results.push({
                            type: 'video',
                            url: src,
                            attributes: { controls: video.getAttribute('controls') || '', autoplay: video.getAttribute('autoplay') || '' }
                        });
                    }
                    video.querySelectorAll('source').forEach((source) => {
                        const src = processUrl(source.getAttribute('src'));
                        if (src) {
                            results.push({ type: 'video', url: src, attributes: { type: source.getAttribute('type') || '' } });
                        }
                    });
                });
            }
            ...
            if (options.audios) {
                doc.querySelectorAll('audio').forEach((audio) => {
                    ...
                    audio.querySelectorAll('source').forEach((source) => {
                        const src = processUrl(source.getAttribute('src'));
                        if (src) {
                            results.push({ type: 'audio', url: src, attributes: { type: source.getAttribute('type') || '' } });
                        }
                    });
                });
            }
...
<body>
    ...
    const handleCopyResults = () => {
        const text = extractedResults.map((item) => `${item.type}: ${item.url}`).join('\n');
        copy(text);
            ...
            <Row gutter={[24, 24]}>
                ...
                <Col xs={24} lg={10}>
                    <Space orientation='vertical'>
                        <Card className="border-none bg-white/5 mb-6"
                            extra={<Space className="w-full justify-between">
                                <Space>
                                    ...
                                    <Button size='small' onClick={handleClear} icon={<DeleteOutlined />} danger ghost><FormattedMessage id="common.clear" /></Button>
                                </Space>
                                <Button size='small' type="primary" onClick={extractContent} icon={<RocketOutlined />}><FormattedMessage id="common.extract" /></Button>
                            </Space>}
                            title={<FormattedMessage id="tools.htmlExtractor.inputTitle" />}>
                            <div className="mb-4">
                                <Text className="block mb-2"><FormattedMessage id="tools.htmlExtractor.baseUrl" /></Text>
                                <Input
                            ...
                            <TextArea
                                ...
                                placeholder={intl.formatMessage({ id: 'tools.htmlExtractor.placeholder' })}
                        ...
                        <Card className="border-none bg-white/5" title={<FormattedMessage id="common.options" />}>
                            <div className="flex justify-between items-center mb-4">
                                <Text strong className="text-slate-300"><FormattedMessage id="tools.htmlExtractor.contentTypes" /></Text>
                                <Space size="small">
                                    <Button size="small" type="text" className="text-primary-400 hover:text-primary-300" onClick={() => toggleAllTypes(true)}><FormattedMessage id="common.selectAll" /></Button>
                                    <Button size="small" type="text" className="text-slate-400 hover:text-slate-300" onClick={() => toggleAllTypes(false)}><FormattedMessage id="common.clear" /></Button>
                                </Space>
                            </div>

                            <Row gutter={[12, 12]} className="mb-6">
                                {Object.keys(options).filter(k => !['uniqueOnly', 'absoluteUrls'].includes(k)).map(key => (
                                    <Col span={12} key={key}>
                                        <Checkbox
                                        ...
                                        >
                                            <FormattedMessage id={`tools.htmlExtractor.option.${key}`} defaultMessage={key} />
                                        </Checkbox>
                                    </Col>
                                ))}
                            </Row>
                            ...
                            <Space direction="vertical" className="w-full">
                                <Checkbox
                                    ...
                                    onChange={e => setOptions({ ...options, uniqueOnly: e.target.checked })}
                                    className="text-slate-300"
                                >
                                ...
                                <Checkbox
                                    ...
                                    onChange={e => setOptions({ ...options, absoluteUrls: e.target.checked })}
                                    className="text-slate-300"
                                >
                    ...
                    >
                        {totalCount === 0 ? (
                            <Empty
                                ...
                                description={<span className="text-slate-500"><FormattedMessage id="tools.htmlExtractor.noResults" /></span>}
                                className="my-12"
                            />
                        ) : (
                            <div className="space-y-4 max-h-[100vh] overflow-y-auto pr-2 custom-scrollbar">
                                {Object.entries(resultsByType).map(([type, items]) => (
                                    <div key={type} className="border border-slate-700/50 rounded-lg overflow-hidden">
                                        <div className="px-3 py-2 border-b border-slate-700/50 flex items-center justify-between">
                                            <span className="font-medium text-sm flex items-center gap-2">
                                                <span>{getTypeEmoji(type)}</span>
                                                <span className="uppercase">{type}</span>
                                                <Tag className="ml-2 border-none text-slate-400">{items.length}</Tag>
                                            </span>
                                        </div>
                                        <div className="divide-y divide-slate-700/30">
                                            {items.map((item, index) => (
                                                <div key={index} className="p-3 hover:transition-colors">
                                                    <div className="mb-1">
                                                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-primary-400 hover:text-primary-300 hover:underline break-all font-mono text-xs">
                                                            {item.url}
                                                        </a>
                                                    </div>
                                                    {item.text && <div className="text-slate-400 text-xs mb-1">{item.text}</div>}

                                                    {/* Attributes */}
                                                    {item.attributes && Object.keys(item.attributes).length > 0 && (
                                                        <div className="flex flex-wrap gap-1 mb-2">
                                                            {Object.entries(item.attributes).map(([k, v]) => (
                                                                <span key={k} className="inline-block text-slate-400 px-1.5 py-0.5 rounded text-[10px]">
                                                                    {k}: {v}
                                                                </span>
                                                            ))}
                                                        </div>
                                                    ...
                                                    {item.type === 'video' && (
                                                        <div className="mt-2 p-1 rounded inline-block">
                                                            <video src={item.url} controls className="max-h-32 max-w-full rounded" onError={(e) => (e.target as HTMLVideoElement).style.display = 'none'} />
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
