        ...
        <div className="max-w-6xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <Label className="text-xs text-muted-foreground mb-2 block">压缩质量: {quality}%</Label>
              ...
              <div>
                <Label className="text-xs text-muted-foreground mb-2 block">最大宽度 (px)</Label>
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                {stats.orig > 0 && (
                  <div className="grid grid-cols-3 gap-4 mb-4 p-3 rounded-lg bg-green-50 border border-green-100">
                    <div className="text-center"><div className="text-lg font-bold text-green-600">{formatSize(stats.orig)}</div><div className="text-xs text-muted-foreground">原始大小</div></div>
                    <div className="text-center"><div className="text-lg font-bold text-green-600">{formatSize(stats.comp)}</div><div className="text-xs text-muted-foreground">压缩后</div></div>
                    <div className="text-center"><div className="text-lg font-bold text-green-600">-{stats.saved}%</div><div className="text-xs text-muted-foreground">节省</div></div>
                  </div>
                )}
                <div className="space-y-2">
                  {images.map((img) => (
                    <div key={img.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:border-purple-200">
            ...
            <DialogContent className="max-w-2xl">
              ...
              {previewItem && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-center mb-2 text-muted-foreground">原始 ({formatSize(previewItem.originalSize)})</p>
                    <div className="bg-muted rounded-lg flex items-center justify-center h-48 border"><img src={previewItem.preview} className="max-w-full max-h-full object-contain" /></div>
                  </div>
                  <div>
                    <p className="text-xs text-center mb-2 text-green-600">压缩后 ({formatSize(previewItem.compressedSize || 0)})</p>
                    <div className="bg-muted rounded-lg flex items-center justify-center h-48 border">{previewUrl && <img src={previewUrl} className="max-w-full max-h-full object-contain" />}</div>
                  </div>
                </div>
              )}
            </DialogContent>
