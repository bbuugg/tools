<template>
  ...
  >
    ...
    <!-- GIF Preview Section -->
    <div v-if="selectedGif" class="glass rounded-xl border border-slate-700/30 p-6 mb-6">
      <h3 class="text-lg font-semibold text-slate-100 mb-4 border-b border-slate-700/30 pb-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <!-- GIF Preview -->
        <div>
          <h4 class="text-md font-semibold text-slate-200 mb-3">
          ...
          <div class="space-y-3 max-h-96 overflow-y-auto">
            ...
            >
              ...
              <div class="ml-3 flex-grow min-w-0">
                ...
                <p class="text-xs text-slate-400">
                  Position: ({{ frame.left }}, {{ frame.top }}) {{ frame.width }}×{{ frame.height }}
                </p>
              </div>
    ...
    <!-- Result Section -->
    <div v-if="generatedGif" class="glass rounded-xl border border-slate-700/30 p-6 mb-6">
      <h3 class="text-lg font-semibold text-slate-100 mb-4 border-b border-slate-700/30 pb-2">
    ...
    <div class="glass rounded-xl border border-slate-700/30 p-6 mb-6">
      ...
      <ul class="space-y-2 text-slate-300">
        <li class="flex items-start">
          ...
          <span>{{ $t('tools.gifEditor.tips.tip1') }}</span>
        </li>
        <li class="flex items-start">
          <span class="mr-2">•</span>
          <span>{{ $t('tools.gifEditor.tips.tip2') }}</span>
        </li>
        <li class="flex items-start">
          <span class="mr-2">•</span>
          <span>{{ $t('tools.gifEditor.tips.tip3') }}</span>
        </li>
        <li class="flex items-start">
          <span class="mr-2">•</span>
          <span>{{ $t('tools.gifEditor.tips.tip4') }}</span>
        </li>
      </ul>
    </div>
  </ToolLayout>
...
function handleFileDrop(event: DragEvent) {
  ...
  if (files && files.length > 0) {
    ...
    if (gifFiles.length > 0) {
      ...
      showErrorToast(t('tools.gifEditor.errors.noGif'))
    }
  }
}
...
async function handleGifFile(file: File) {
  if (file.type !== 'image/gif') {
    showErrorToast(t('tools.gifEditor.errors.invalidFile'))
    return
  }

  if (file.size > 50 * 1024 * 1024) {
    // 50MB limit
    showErrorToast(t('tools.gifEditor.errors.fileTooLarge'))
    return
  }
  ...
  try {
    ...
    showErrorToast(t('tools.gifEditor.errors.fileProcessing'))
  }
}
...
async function parseGifFrames(file: File) {
  try {
    ...
    showErrorToast(t('tools.gifEditor.errors.frameParsingFailed'))
  }
}
...
async function generateGif() {
  if (!selectedGif.value || frames.value.length === 0) {
    showErrorToast(t('tools.gifEditor.errors.noFrames'))
    return
  }
  ...
  try {
    ...
    showErrorToast(t('tools.gifEditor.errors.processingFailed') + ': ' + (err as Error).message)
  } finally {
