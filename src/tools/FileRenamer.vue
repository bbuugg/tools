  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-6xl mx-auto px-4">
      ...
      <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $t('fileRenamer.title') }}</h1>
        <p class="text-gray-600">{{ $t('fileRenamer.subtitle') }}</p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              <p class="text-lg font-medium text-gray-700">
                {{ $t('fileRenamer.uploadArea.title') }}
              </p>
              <p class="text-sm text-gray-500">{{ $t('fileRenamer.uploadArea.subtitle') }}</p>
            </div>
            <div>
              <input
                type="file"
              ...
              >
                {{ $t('fileRenamer.uploadArea.selectFiles') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-if="files.length > 0" class="mt-4 text-sm text-gray-600">
          {{ $t('fileRenamer.fileCount', { count: files.length }) }}
        </div>
      </div>
      ...
      <div v-if="files.length > 0" class="bg-white rounded-lg shadow-md mb-6">
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.sequential.prefix') }}
                </label>
                <input
                  v-model="sequentialOptions.prefix"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :placeholder="$t('fileRenamer.sequential.prefixPlaceholder')"
                />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.sequential.startNumber') }}
                </label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.sequential.padding') }}
                </label>
                <input
          ...
          <div v-if="activeTab === 'replace'" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.replace.findText') }}
                </label>
                <input
                  v-model="replaceOptions.findText"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :placeholder="$t('fileRenamer.replace.findPlaceholder')"
                />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.replace.replaceText') }}
                </label>
                <input
                  v-model="replaceOptions.replaceText"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :placeholder="$t('fileRenamer.replace.replacePlaceholder')"
                />
              </div>
            </div>
            <div class="flex items-center">
              ...
              <label for="case-sensitive" class="ml-2 block text-sm text-gray-700">
                {{ $t('fileRenamer.replace.caseSensitive') }}
              </label>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'case'" class="space-y-4">
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700">
                {{ $t('fileRenamer.case.transformation') }}
              </label>
              <div class="space-y-2">
          ...
          <div v-if="activeTab === 'insert'" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.insert.text') }}
                </label>
                <input
                  v-model="insertOptions.text"
                  type="text"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  :placeholder="$t('fileRenamer.insert.textPlaceholder')"
                />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.insert.position') }}
                </label>
                <select
                  v-model="insertOptions.position"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="prefix">{{ $t('fileRenamer.insert.prefix') }}</option>
                  <option value="suffix">{{ $t('fileRenamer.insert.suffix') }}</option>
                  <option value="index">{{ $t('fileRenamer.insert.atIndex') }}</option>
                </select>
              </div>
            </div>
            <div v-if="insertOptions.position === 'index'" class="w-full md:w-1/2">
              <label class="block text-sm font-medium text-gray-700 mb-2">
                {{ $t('fileRenamer.insert.index') }}
              </label>
              <input
          ...
          <div v-if="activeTab === 'truncate'" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.truncate.startIndex') }}
                </label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  {{ $t('fileRenamer.truncate.endIndex') }}
                </label>
                <input
            ...
            <p class="text-sm text-gray-500">{{ $t('fileRenamer.truncate.description') }}</p>
          </div>
        </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
          <div class="flex space-x-4">
            ...
            >
              {{ $t('fileRenamer.actions.download') }}
            </button>
            <button
              @click="clearFiles"
              class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition-colors"
            >
              {{ $t('fileRenamer.actions.clear') }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="files.length > 0" class="bg-white rounded-lg shadow-md">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-medium text-gray-900">{{ $t('fileRenamer.fileList.title') }}</h3>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                >
                  {{ $t('fileRenamer.fileList.originalName') }}
                </th>
                <th
                  v-if="hasPreview"
                  class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  {{ $t('fileRenamer.fileList.newName') }}
                </th>
                <th
                  class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  {{ $t('fileRenamer.fileList.size') }}
                </th>
                <th
                  class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                >
                  {{ $t('fileRenamer.fileList.type') }}
                </th>
              </tr>
            </thead>
